...
import { Play, Clock, Sparkles, FileText, ChevronDown, ChevronUp, Eye } from 'lucide-react';
import { KeyframeItem } from '@/types';
import { useVideoStore } from '@/lib/store';
import { formatTimestamp } from '@/lib/utils';
import { VLMInspectorModal } from './VLMInspectorModal';
...
export const KeyframeSearchResult: React.FC<KeyframeSearchResultProps> = ({ keyframe }) => {
  ...
  const [isVlmOpen, setIsVlmOpen] = useState<boolean>(false);
  ...
  return (
    <>
        ...
        <div className="p-3 bg-black/90 text-xs space-y-2">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="text-[10px] font-bold text-brand-neon bg-brand-neon/15 hover:bg-brand-neon/30 border border-brand-neon/40 px-2 py-0.5 rounded-pill flex items-center gap-1 transition-all"
            >
              <Eye className="w-3 h-3" /> VLM Deep-Dive
            </button>
          </div>
      ...
      {/* VLM Deep Visual Reasoning Modal */}
      <VLMInspectorModal
        isOpen={isVlmOpen}
