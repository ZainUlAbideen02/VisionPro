...
def run_e2e_vector_verification():
    ...
    print(f"[✔] Query 1 Text Vector Dimension: {len(v_text1)}")
    print(f"[✔] Query 2 Text Vector Dimension: {len(v_text2)}")
    ...
    print(f"[✔] Vector 1 L2 Norm: {norm_text1:.4f} (Cosine normalized)")
    ...
    print(f"[✔] Keyframe Image Vector Dimension: {len(v_img)}")
    assert len(v_img) == 768, f"Expected 768-dim vector, got {len(v_img)}"

    # 3. Compute Cosine Similarity (Dot Product of Unit Vectors)
    similarity_red = np.dot(v_text1, v_img)
    similarity_code = np.dot(v_text2, v_img)

    print(f"\n[📊] Cosine Similarity ('red server terminal' vs Red Image): {similarity_red:.4f}")
    print(f"[📊] Cosine Similarity ('code text' vs Red Image): {similarity_code:.4f}")
